...
namespace CD4_Server.Comm
{
    class Svr
    {
        ...
        public Svr(string ip, int port, Action<string> guiUpdater)
        {
        ...
        private void Accept()
        {
            while (acceptingThread.IsAlive)
            {
                try
                {
                    clients.Add(new User(serverSocket.Accept(), new Action<string, Socket>(NewMessageReceived)));
                }
                catch (Exception e)
                {
